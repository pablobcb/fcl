...
/**********************************************************************************************************/
    template< 
        class IN_TYPE, 
        template < class IN_TYPE, class A = std::allocator< IN_TYPE > > class COLLECTION
    > COLLECTION < IN_TYPE > take (const COLLECTION< IN_TYPE >& collection, int size )
    {
        COLLECTION < IN_TYPE > result;

        typename COLLECTION< IN_TYPE >::const_iterator it;
        unsigned int i = 0; 
        for( it = collection.begin() ; it != collection.end() && i < size ; ++it )
        {
            IN_TYPE node = *it;
    ...
    template< 
        class IN_TYPE, 
        template < class IN_TYPE, class A = std::allocator< IN_TYPE > > class COLLECTION
    > COLLECTION < IN_TYPE > drop (const COLLECTION< IN_TYPE >& collection, int size )
    {
        COLLECTION < IN_TYPE > result;
        ...
        for( std::advance(it, size)  ; it != collection.end() ; ++it )
        {
            IN_TYPE node = *it;
